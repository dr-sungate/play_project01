...
trait BaseAuthConfig  extends AuthConfig {
  ...
  def resolveUser(id: Id)(implicit ctx: ExecutionContext): Future[Option[AccountsRow]] = UserAccountSv.findById(id)
  ...
  def authorize(user: User, authority: Authority)(implicit ctx: ExecutionContext) = Future.successful((Role.valueOf(user.role), authority) match {
    ...
    case (AccountingUser, AccountingUser) => true
    case _ => false
  })
